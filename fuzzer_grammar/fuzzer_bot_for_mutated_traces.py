...
def generate_pcap_with_session_id_mutation(packet_data, session_id):
    """
    Modify the session ID in a given PPPoE packet.

    :param packet_data: Raw binary packet data (bytes)
    :param session_id: New session ID to assign (int)
    :return: Modified Scapy packet
    """

    # Convert raw bytes to a Scapy packet
    original_packet = Ether(packet_data)

    # Ensure it contains a PPPoE layer
    if not original_packet.haslayer(PPPoE):
        raise ValueError("The provided data does not contain a valid PPPoE packet.")

    # Modify the session ID while keeping everything else the same
    original_packet[PPPoE].sessionid = session_id

    return original_packet  # Return the modified packet
...
def generate_padt_with_session_id(padt_packet, new_session_id):
    """
    Replace the session_id field in a PADT packet (raw bytes).

    :param padt_packet: Raw binary packet data (bytes)
    :param new_session_id: New session ID to write (integer)
    :return: Modified packet as bytes
    """
    if len(padt_packet) < 4:
        raise ValueError("Invalid PADT packet: Too short to contain a session ID.")
    ...
    # Replace the session ID at offset 2 (after version_type + code)
    modified_packet = padt_packet[:2] + new_session_id_bytes + padt_packet[4:]

    return modified_packet  # Return the modified binary packet

# Main fuzzer bot functionality
def fuzzer_bot_mutation(interface, padi_generated_file, padr_generated_file, configure_request_generated_file, echo_request_generated_file, authentication_request_generated_file, padt_generated_file, source_addr= "11:22:33:44:55:66"):
    ...
    send_packet(sock, interface, padi_generated_file)
    #INTERESTING
    pado_response = listen_for_response(sock)
    while(packet_type(pado_response)!="PADO"):
        send_packet(sock, interface, padi_generated_file)
        pado_response = listen_for_response(sock)
    ...
    send_packet(sock, interface, padr_generated_file)
    pads_response = listen_for_response(sock)
    while(packet_type(pads_response)!="PADS"):
        send_packet(sock, interface, padr_generated_file)
        pads_response = listen_for_response(sock)
    ...
    configure_request_packet= generate_pcap_with_session_id_mutation(configure_request_generated_file, session_id)
    ...
    ##ECHO REQUEST
    echo_request_packet= generate_pcap_with_session_id_mutation(echo_request_generated_file, session_id)
    try:
        ...
        print(f"Error reading ECHO REQ packet: {e}")
        return "",[],[] ##temporary
    ...
    authentication_request_pcap_file= generate_pcap_with_session_id_mutation(authentication_request_generated_file, session_id)
    # try:
    ...
    # except Exception as e:
    #     print(f"Error reading AUTHENTICATION REQ packet: {e}")
    #     return [],[],[] ##temporary
    send_packet(sock, interface, authentication_request_pcap_file, protocol=ETH_P_PPP_SESS)
    lcp_response = listen_for_response(sock)
   ...
   # sock = socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.htons(ETH_P_PPP_SESS))
    ...
    ##PADT 
    padt_generated_file_with_session_id = generate_padt_with_session_id(padt_generated_file, session_id)

    # try:
    #     #padt_packet = read_packet_from_pcap(padt_pcap, index=0)  # Assuming first packet is PADI
    #     print(f"PADT Packet: {padt_generated_file_with_session_id.summary()}")
    # except Exception as e:
    #     print(f"Error reading PADT packet: {e}")
    #     return "",[],[] ##temporary

    send_packet(sock, interface, padt_generated_file_with_session_id, protocol=ETH_P_PPP_SESS )
    #lcp_response = listen_for_response(sock)

    sock.close()
    print(f"Fuzzer bot operation completed. Logs saved in {LOG_FILE}")
    return trace, input_packet_sequence, mutate_packet_sequence ##temporary
